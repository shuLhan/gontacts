...
//
// SetAddresses will set contact address using value from list of gontacts
// Address.
//
func (c *Contact) SetAddresses(addresses []gontacts.Address) {
	for _, adr := range addresses {
		switch adr.Type {
		case gontacts.TypeWork:
...
//
func (c *Contact) SetEmails(emails []gontacts.Email) {
...
//
// SetName will set Proofn contact name according to value in gontacts Name.
//
func (c *Contact) SetName(name *gontacts.Name) {
	if name == nil {
		return
	}
...
//
// SetPhones will set Proofn contact phone based on gontacts Phone type and
// number.
//
func (c *Contact) SetPhones(phones []gontacts.Phone) {
	for _, phone := range phones {
		switch phone.Type {
		case gontacts.TypeWork:
			c.WorkPhone = phone.Number

		case gontacts.TypeMobile:
